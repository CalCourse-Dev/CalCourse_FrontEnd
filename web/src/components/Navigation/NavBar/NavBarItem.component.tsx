...
const NavBarItem = (item: INavBarItem) => {
    ...
    return (
        ...
        <li
            className={`my-auto group h-12 text-graphite flex gap-4 flex-row-reverse sm:flex-row mb:justify-center mb:items-center duration-150 ${
                user_logged_in ? 'cursor-pointer' : 'cursor-default'
            }  ${selected ? 'opacity-100' : 'opacity-30 hover:opacity-70'}`}
            key={item.label}
            onClick={() => {
                if (user_logged_in) {
                    navigate(item.path)
        ...
        >
            <h2 className="text-xl text-right leading-none m-0 font-bold mb:hidden sm:hidden mb:group-hover:block sm:group-hover:block mb:fixed sm:fixed mb:mb-[7.5rem] sm:ml-14 mb:card-transluscent sm:card-transluscent mb:p-2 sm:p-2 rounded-2xl w-max order-2">
                {item.label}
            </h2>
